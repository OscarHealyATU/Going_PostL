    ...
    void Start()
    {
        ...
        for (float x = 0; x < noOfHousesX; x++)
        {
            for (float z = 0; z < noOfHousesZ; z++)
            {
                ...
                // instanciates tiles
                GameObject groundInstSqr = Instantiate(groundSquare, position, Quaternion.Euler(-90, 0, 0), transform);
                groundInstSqr.transform.localScale = houseScale;
                ...
                if (!isWarehousePosition)
                {
                    ...
                    // fixing large scale issues and adding natural scale and rotation
